...
function ensureEmployeeSchema(PDO $pdo): void {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS employee_face_profiles (
        face_profile_id INT AUTO_INCREMENT PRIMARY KEY, employee_id INT NOT NULL UNIQUE,
        face_descriptor JSON NOT NULL, reference_photo_path VARCHAR(255) NOT NULL,
        blink_verified TINYINT(1) NOT NULL DEFAULT 1, enrolled_by INT NOT NULL,
...
if ($action === 'list') {
    ...
    $stmt = $pdo->query("SELECT id, full_name, email, contact_no, username, position, status, created_at,
                         EXISTS(SELECT 1 FROM employee_face_profiles fp WHERE fp.employee_id = employees.id) AS face_enrolled
...
if ($action === 'enroll_face' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isAdmin()) {
        http_response_code(403);
    ...
    $descriptor = json_decode(trim($_POST['face_descriptor'] ?? ''), true);
    $photo = trim($_POST['face_photo'] ?? '');
    if (($_POST['liveness_verified'] ?? '') !== '1' || !is_array($descriptor) || count($descriptor) !== 128) {
        echo json_encode(['success' => false, 'message' => 'Complete the employee face and head-turn verification.']);
        exit;
    }
    foreach ($descriptor as $value) {
        if (!is_numeric($value) || !is_finite((float)$value) || abs((float)$value) > 5) {
            echo json_encode(['success' => false, 'message' => 'The captured face profile is invalid.']);
            exit;
        }
    }
    if (!preg_match('#^data:image/jpeg;base64,([A-Za-z0-9+/=]+)$#', $photo, $match)) {
        echo json_encode(['success' => false, 'message' => 'A valid live face photo is required.']);
        exit;
    }
    $binary = base64_decode($match[1], true);
    if ($binary === false || strlen($binary) < 1000 || strlen($binary) > 3 * 1024 * 1024) {
        echo json_encode(['success' => false, 'message' => 'The live face photo is invalid or too large.']);
        exit;
    }
    $employee = $pdo->prepare("SELECT id FROM employees WHERE id = ? AND status = 'approved' AND position <> 'Administrator' AND position NOT LIKE '%HR%' LIMIT 1");
    $employee->execute([$employeeId]);
    if (!$employee->fetch()) {
        echo json_encode(['success' => false, 'message' => 'Select an active employee account.']);
        exit;
    }
    $directory = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'face_profiles';
    if (!is_dir($directory) && !mkdir($directory, 0755, true)) {
        echo json_encode(['success' => false, 'message' => 'Unable to prepare face profile storage.']);
        exit;
    }
    $fileName = bin2hex(random_bytes(24)) . '.jpg';
    $absolutePath = $directory . DIRECTORY_SEPARATOR . $fileName;
    if (file_put_contents($absolutePath, $binary, LOCK_EX) === false) {
        echo json_encode(['success' => false, 'message' => 'Unable to save the face profile.']);
        exit;
    }
    $relativePath = 'uploads/face_profiles/' . $fileName;
    $old = $pdo->prepare('SELECT reference_photo_path FROM employee_face_profiles WHERE employee_id = ?');
    $old->execute([$employeeId]);
    $oldPath = $old->fetchColumn();
    $stmt = $pdo->prepare("INSERT INTO employee_face_profiles (employee_id, face_descriptor, reference_photo_path, blink_verified, enrolled_by)
        VALUES (?, ?, ?, 1, ?) ON DUPLICATE KEY UPDATE face_descriptor=VALUES(face_descriptor), reference_photo_path=VALUES(reference_photo_path), blink_verified=1, enrolled_by=VALUES(enrolled_by), updated_at=NOW()");
    $stmt->execute([$employeeId, json_encode(array_map('floatval', $descriptor)), $relativePath, (int)$_SESSION['user_id']]);
    if ($oldPath) {
        $oldAbsolute = dirname(__DIR__) . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $oldPath);
        if (is_file($oldAbsolute)) unlink($oldAbsolute);
    }
    echo json_encode(['success' => true, 'message' => 'Employee face and head-movement profile saved successfully.']);
    exit;
}
