...
if (isLimitedEmployeeSession($pdo)) {
    ...
    payrollRespond(false, 'Payslips unlock after final Manager confirmation.');
}
...
function payrollEnsureSchema(PDO $pdo): void {
    ...
    $paymentStatusColumn = $pdo->query("SHOW COLUMNS FROM payroll_records LIKE 'payment_status'")->fetch();
    if (!$paymentStatusColumn) $pdo->exec("ALTER TABLE payroll_records ADD COLUMN payment_status ENUM('pending','paid') NOT NULL DEFAULT 'paid' AFTER status");
}
...
function payrollEmployees(PDO $pdo): array {
    $stmt = $pdo->query("SELECT id, full_name, email, position, created_at FROM employees WHERE status = 'approved' AND position <> 'Administrator' AND position NOT LIKE '%HR%' AND position NOT LIKE '%Finance%' ORDER BY full_name");
...
if ($action === 'history') {
    ...
    $stmt = $pdo->query("SELECT period, COUNT(*) AS employees, COALESCE(SUM(gross_pay), 0) AS total_gross,
        COALESCE(SUM(total_deductions), 0) AS total_deductions, COALESCE(SUM(net_pay), 0) AS total_net,
        MAX(issued_at) AS issued_at
        FROM payroll_records
        WHERE status = 'issued' AND period REGEXP '^[0-9]{4}-[0-9]{2}-(15|30)$'
        GROUP BY period
        ORDER BY period DESC");
    $records = $stmt->fetchAll();
    foreach ($records as &$record) $record['period_label'] = payrollPeriodLabel($record['period']);
    unset($record);
    payrollRespond(true, '', ['records' => $records]);
}

if ($action === 'history_details') {
    if ($user['position'] !== 'Administrator') {
        http_response_code(403);
        payrollRespond(false, 'Administrator access is required.');
    }
    $period = payrollPeriod($_GET['period'] ?? '');
    $stmt = $pdo->prepare("SELECT pr.employee_id, pr.period, pr.gross_pay, pr.total_deductions, pr.net_pay,
        e.full_name, e.email, e.position
        FROM payroll_records pr
        JOIN employees e ON e.id = pr.employee_id
        WHERE pr.period = ? AND pr.status = 'issued'
        ORDER BY e.full_name");
    $stmt->execute([$period]);
    $records = $stmt->fetchAll();
    payrollRespond(true, '', ['period' => $period, 'period_label' => payrollPeriodLabel($period), 'records' => $records]);
}
