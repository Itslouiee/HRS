...
function isLimitedEmployeeSession(PDO $pdo): bool {
    $userId = (int)($_SESSION['user_id'] ?? 0);
    if ($userId < 1) return false;
    $stmt = $pdo->prepare("SELECT access_level, position FROM employees WHERE id = ? LIMIT 1");
    $stmt->execute([$userId]);
    $row = $stmt->fetch();
    if (!$row) return false;
    $position = (string)$row['position'];
    $privileged = $position === 'Administrator' || stripos($position, 'HR') !== false || stripos($position, 'manager') !== false || stripos($position, 'finance') !== false;
    return !$privileged && ($row['access_level'] ?? 'full') === 'limited';
}