...
<head>
  ...
  <style>
    body{margin:0;min-height:100vh;display:grid;place-items:center;background:#2B1F18;color:#E6D6C2;font-family:Arial,sans-serif}
    .box{width:min(92vw,460px);padding:34px;border:1px solid rgba(201, 161, 122,.35);border-radius:18px;background:#2B1F18;text-align:center;box-shadow:0 22px 70px rgba(43, 31, 24,.35)}
    .icon{width:64px;height:64px;margin:0 auto 18px;border-radius:50%;display:grid;place-items:center;background:#C9A17A;color:#2B1F18;font-size:32px;font-weight:700}
    h1{margin:0 0 12px;font-family:Georgia,serif;font-size:30px;color:#C9A17A}
    p{margin:0;line-height:1.65;color:rgba(230, 214, 194,.76)}
  </style>
</head>
<body>
  <main class="box">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isLimitedEmployeeSession($pdo)) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Profile and PIN changes unlock after final Manager confirmation.']);
        exit;
    }
    $action = $_POST['action'] ?? '';
